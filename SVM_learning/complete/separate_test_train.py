...
def separate_no_clips(inp):
    ...
    np.save('/media/Hao/My Book/raw_total_fv/total_training' + inp, fv_total)
    np.save('/home/Hao/Work/Cmts/raw/total_training' + inp, fv_total)

    Fir = True
    for i in xrange(len(testing_fv)):
        print 'case ' + str(i)
        fv = np.load(testing_fv[i])
        if Fir == True:
            fv_total = fv
            Fir = False
        else:
    ...
    np.save('/media/Hao/My Book/raw_total_fv/total_testing' + inp, fv_total)
    np.save('/home/Hao/Work/Cmts/raw/total_testing' + inp, fv_total)

def separate(inp):
    f = open('/home/Hao/Work/Cmts/raw_our_UW_list.txt', 'r')
    video_list = []
    video_list = get_video_list(f, video_list)
    input_list = []
    for i in video_list:
    ...
    np.savez('/home/Hao/Work/Cmts/raw/total_training' + inp, fv = fv_total, label = label_total, q = q_total)
    ...
    np.savez('/home/Hao/Work/Cmts/raw/total_testing' + inp, fv = fv_total, label = label_total, q = q_total)

separate_no_clips('7_4_22')
